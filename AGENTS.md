...
## Gmail Setup
Gmail OAuth credentials are stored outside the repo. The scan service reads the token from the path set in `GMAIL_TOKEN_FILE` (in `.env` or environment). Run `backend/scripts/generate_token.py` once to perform OAuth consent and cache the token. On Render, base64-encode `token.json` and set it as the `GMAIL_TOKEN_JSON` secret — the app writes it to disk at startup.
