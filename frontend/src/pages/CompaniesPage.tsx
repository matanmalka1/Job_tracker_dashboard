        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-0.5">
            ...
            <span className="text-gray-600 text-xs">
              Last: {formatDate(group.latestActivity)}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            {STATUS_ORDER.filter((s) => group.statusCounts[s]).map((s) => (
              <div
                key={s}
                className={['w-2 h-2 rounded-full', STATUS_DOT[s]].join(' ')}
                title={`${s}: ${group.statusCounts[s]}`}
              />
            ))}
          </div>
      ...
      {expanded && (
        <div className="border-t border-white/5">
          {group.applications.map((app, i) => (
            ...
            >
              <div
                className={['w-1.5 h-1.5 rounded-full shrink-0', STATUS_DOT[app.status]].join(' ')}
              />
              <div className="flex-1 min-w-0">
                <p className="text-gray-300 text-sm truncate">{app.role_title}</p>
                {app.source && (
                  <p className="text-gray-600 text-xs mt-0.5">{app.source}</p>
                )}
              </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">
          {data
            ? `${companyCount} compan${companyCount !== 1 ? 'ies' : 'y'} · ${data.total} application${data.total !== 1 ? 's' : ''}`
            : 'Loading…'}
        </p>
      </div>
      ...
      {!isLoading && !isError && groups.length === 0 && (
        <div className="bg-[#1a1a24] rounded-xl p-12 border border-white/5 text-center">
          ...
          <p className="text-gray-600 text-xs mt-1">
            Add applications to see them grouped by company.
          </p>
        </div>
      )}
