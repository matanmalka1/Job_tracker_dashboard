    ...
    >
      ...
      <div className="relative bg-[#1a1a24] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button type="button" onClick={onClose} className="text-gray-500 hover:text-white transition-colors" aria-label="Close">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="date" value={form.applied_at} onChange={set('applied_at')} className={`${inputCls} [color-scheme:dark]`} />
          </div>
          ...
          <div>
            ...
            <input type="date" value={form.next_action_at} onChange={set('next_action_at')} className={`${inputCls} [color-scheme:dark]`} />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} disabled={loading} className="flex-1 px-4 py-2.5 rounded-lg border border-white/10 text-gray-400 text-sm font-medium hover:text-white hover:border-white/20 transition-colors disabled:opacity-50">
              Cancel
            </button>
            <button type="submit" disabled={loading} className="flex-1 px-4 py-2.5 rounded-lg bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white text-sm font-medium transition-colors">
